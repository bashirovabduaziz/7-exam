...
import { carts } from '../../static/data'; // Importing the predefined images
...
const Cart = () => {
    ...
    let items = cartItems?.map((el) => {
        const product = carts.find(item => item.id === el.id);
        return (
            <div key={el.id} className='w-[700px] flex items-center justify-around bg-clr mt-[20px] rounded-[5px] h-[80px]'>
                <img src={product.image_url} alt={product.alt} className='w-[60px] h-[60px]' />
                <Link to={`/single-product/${el.id}`}>
                    <p className='titles font-[500] text-[#3D3D3D]'>{product.common_name}</p>
                </Link>
            ...
            <div className='mx-auto flex items-center justify-between w-[1200px]'>
                {
                    cartItems.length ?
                        ...
                        <div className='product__card'>
                            <img src="https://png.pngtree.com/png-vector/20190719/ourmid/pngtree-empty-box-icon-for-your-project-png-image_1548720.jpg" alt="Empty Cart" />
